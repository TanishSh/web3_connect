...
iScroll deploys ISCROL, an ERC20 token implementing a 24 hr time lock owners can choose to place onto their tokens.

Tag deploys TAG, an ERC721 contract allowing holders of ISCROL to mint TAG, and allowing TAG owners certain permissions.
...
/**
 * @title Infinite Scroll Token
 * @dev This contract deploys the ISCROL ERC20 token with an added 24hr time lock feature for participants.
 */
contract iScroll is ERC20 {
     ...
     */
    constructor() ERC20("iScroll", "ISCROL") {
...
contract Tag is ERC721Enumerable {
    ...
    iScroll public iScrollContract;
    /**
     * @dev Represents the details of a tag.
     */
    struct TagDetails {
        uint256 ethFee;            // Fee in ETH to use this tag
        uint256 tokenRequirement;   // Amount of iScrollContract required to hold for using this tag
    ...
    /**
     * @dev Initializes the Tag contract with a ISCROL token contract address.
     * @param _iScrollContract The address of the ISCROL token contract.
     */
    constructor(address _iScrollContract) ERC721("TagToken", "TAG") {
        iScrollContract = iScroll(_iScrollContract);
    }
    ...
    function createTag(string memory tagContent) public {
        require(iScrollContract.balanceOf(msg.sender) > 10, "Must hold at least ten ISCROL tokens");
    ...
    function updateTagFee(string memory tagContent, uint256 newEthFee) public {
        require(iScrollContract.balanceOf(msg.sender) > 0, "Must hold at least one ISCROL token");
        uint256 tokenId = uint256(keccak256(bytes(tagContent)));
    ...
    /**
     * @dev Updates the ISCROL token requirement for using a tag.
     * @param tagContent The content of the tag.
     * @param newTokenRequirement The new ISCROL token requirement.
     */
    ...
    function updateTokenRequirement(string memory tagContent, uint256 newTokenRequirement) public {
        ...
        require(ownerOf(tokenId) != address(0), "Tag does not exist");
        require(ownerOf(tokenId) == msg.sender, "You are not the owner");
        require(iScrollContract.balanceOf(msg.sender) >= newTokenRequirement);
     ...
     * @param tagContent The content of the tag
     * @return The ETH fee and ISCROL token holding requirement for the tag
     */
 ...
 * It is designed to work in tandem with two other contracts: iScroll and Tag 
...
contract Forum {
     ...
     * @param _iScrollContract Address of the iScroll contract.
     * @param _iScrollContract Address of the Tag contract.
     */

    constructor(address _iScrollContract, address _tagContract) {
        iScrollContract = iScroll(_iScrollContract);
    ...
    function tagPost(uint256 postId, string calldata tagContent) public {
        ...
        require(iScrollContract.balanceOf(msg.sender) >= TagContract.getTokenRequirement(tagContent), "Must hold more ISCROL tokens.");
    ...
    function upvotePost(uint256 postId) public {
        require(iScrollContract.balanceOf(msg.sender) > 0, "Must hold at least one ISCROL token to vote");
        ...
        uint256 amount = iScrollContract.balanceOf(msg.sender);
        iScrollContract.voteLock();
    ...
    function removeUpvote(uint256 postId) public {
        ...
        require(iScrollContract.balanceOf(msg.sender) >= posts[postId].upvotes[msg.sender], "Must hold at least as many ISCROL tokens as the upvotes you are removing");
        iScrollContract.voteLock();
    ...
    function removeDownvote(uint256 postId) public {
        ...
        require(iScrollContract.balanceOf(msg.sender) >= posts[postId].downvotes[msg.sender], "Must hold at least as many ISCROL tokens as the downvotes you are removing");
        iScrollContract.voteLock();
