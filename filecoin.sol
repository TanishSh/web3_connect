...
fileForum deploys FIFO, an ERC20 token implementing a 24 hr time lock owners can choose to place onto their tokens.

Tag deploys TAG, an ERC721 contract allowing holders of FIFO to mint TAG, and allowing TAG owners certain permissions.
...
/**
 * @title File Forum Token
 * @dev This contract deploys the FIFO ERC20 token with an added 24hr time lock feature for participants.
 */
contract fileForum is ERC20 {
     ...
     */
    constructor() ERC20("fileForum", "FIFO") {
...
contract Tag is ERC721Enumerable {
    ...
    fileForum public fileForumContract;
    /**
     * @dev Represents the details of a tag.
     */
    struct TagDetails {
        uint256 ethFee;            // Fee in ETH to use this tag
        uint256 tokenRequirement;   // Amount of fileForumContract required to hold for using this tag
    ...
    /**
     * @dev Initializes the Tag contract with a FIFO token contract address.
     * @param _fileForumContract The address of the FIFO token contract.
     */
    constructor(address _fileForumContract) ERC721("TagToken", "TAG") {
        fileForumContract = fileForum(_fileForumContract);
    }
    ...
    function createTag(string memory tagContent) public {
        require(fileForumContract.balanceOf(msg.sender) > 10, "Must hold at least ten FIFO tokens");
    ...
    function updateTagFee(string memory tagContent, uint256 newEthFee) public {
        require(fileForumContract.balanceOf(msg.sender) > 0, "Must hold at least one FIFO token");
        uint256 tokenId = uint256(keccak256(bytes(tagContent)));
    ...
    /**
     * @dev Updates the FIFO token requirement for using a tag.
     * @param tagContent The content of the tag.
     * @param newTokenRequirement The new FIFO token requirement.
     */
    ...
    function updateTokenRequirement(string memory tagContent, uint256 newTokenRequirement) public {
        ...
        require(ownerOf(tokenId) != address(0), "Tag does not exist");
        require(ownerOf(tokenId) == msg.sender, "You are not the owner");
        require(fileForumContract.balanceOf(msg.sender) >= newTokenRequirement);
     ...
     * @param tagContent The content of the tag
     * @return The ETH fee and FIFO token holding requirement for the tag
     */
 ...
 * It is designed to work in tandem with two other contracts: fileForum and Tag 
...
contract Forum {
     ...
     * @param _fileForumContract Address of the fileForum contract.
     * @param _fileForumContract Address of the Tag contract.
     */

    constructor(address _fileForumContract, address _tagContract) {
        fileForumContract = fileForum(_fileForumContract);
    ...
    function tagPost(uint256 postId, string calldata tagContent) public {
        ...
        require(fileForumContract.balanceOf(msg.sender) >= TagContract.getTokenRequirement(tagContent), "Must hold more FIFO tokens.");
    ...
    function upvotePost(uint256 postId) public {
        require(fileForumContract.balanceOf(msg.sender) > 0, "Must hold at least one FIFO token to vote");
        ...
        uint256 amount = fileForumContract.balanceOf(msg.sender);
        fileForumContract.voteLock();
    ...
    function removeUpvote(uint256 postId) public {
        ...
        require(fileForumContract.balanceOf(msg.sender) >= posts[postId].upvotes[msg.sender], "Must hold at least as many FIFO tokens as the upvotes you are removing");
        fileForumContract.voteLock();
    ...
    function removeDownvote(uint256 postId) public {
        ...
        require(fileForumContract.balanceOf(msg.sender) >= posts[postId].downvotes[msg.sender], "Must hold at least as many FIFO tokens as the downvotes you are removing");
        fileForumContract.voteLock();
