...
infiniteScroll deploys BAST, an ERC20 token implementing a 24 hr time lock owners can choose to place onto their tokens.

Tag deploys TAG, an ERC721 contract allowing holders of BAST to mint TAG, and allowing TAG owners certain permissions.
...
/**
 * @title Base Talk Token
 * @dev This contract deploys the BAST ERC20 token with an added 24hr time lock feature for participants.
 */
contract baseTalk is ERC20 {
     ...
     */
    constructor() ERC20("baseTalk", "BAST") {
...
contract Tag is ERC721Enumerable {
    ...
    baseTalk public baseTalkContract;
    /**
     * @dev Represents the details of a tag.
     */
    struct TagDetails {
        uint256 ethFee;            // Fee in ETH to use this tag
        uint256 tokenRequirement;   // Amount of baseTalkContract required to hold for using this tag
    ...
    /**
     * @dev Initializes the Tag contract with a BAST token contract address.
     * @param _baseTalkContract The address of the BAST token contract.
     */
    constructor(address _baseTalkContract) ERC721("TagToken", "TAG") {
        baseTalkContract = baseTalk(_baseTalkContract);
    }
    ...
    function createTag(string memory tagContent) public {
        require(baseTalkContract.balanceOf(msg.sender) > 10, "Must hold at least ten BAST tokens");
    ...
    function updateTagFee(string memory tagContent, uint256 newEthFee) public {
        require(baseTalkContract.balanceOf(msg.sender) > 0, "Must hold at least one BAST token");
        uint256 tokenId = uint256(keccak256(bytes(tagContent)));
    ...
    /**
     * @dev Updates the BAST token requirement for using a tag.
     * @param tagContent The content of the tag.
     * @param newTokenRequirement The new BAST token requirement.
     */
    ...
    function updateTokenRequirement(string memory tagContent, uint256 newTokenRequirement) public {
        ...
        require(ownerOf(tokenId) != address(0), "Tag does not exist");
        require(ownerOf(tokenId) == msg.sender, "You are not the owner");
        require(baseTalkContract.balanceOf(msg.sender) >= newTokenRequirement);
     ...
     * @param tagContent The content of the tag
     * @return The ETH fee and BAST token holding requirement for the tag
     */
 ...
 * It is designed to work in tandem with two other contracts: baseTalk and Tag 
...
contract Forum {
     ...
     * @param _baseTalkContract Address of the baseTalk contract.
     * @param _baseTalkContract Address of the Tag contract.
     */

    constructor(address _baseTalkContract, address _tagContract) {
        baseTalkContract = baseTalk(_baseTalkContract);
    ...
    function tagPost(uint256 postId, string calldata tagContent) public {
        ...
        require(baseTalkContract.balanceOf(msg.sender) >= TagContract.getTokenRequirement(tagContent), "Must hold more BAST tokens.");
    ...
    function upvotePost(uint256 postId) public {
        require(baseTalkContract.balanceOf(msg.sender) > 0, "Must hold at least one BAST token to vote");
        ...
        uint256 amount = baseTalkContract.balanceOf(msg.sender);
        baseTalkContract.voteLock();
    ...
    function removeUpvote(uint256 postId) public {
        ...
        require(baseTalkContract.balanceOf(msg.sender) >= posts[postId].upvotes[msg.sender], "Must hold at least as many BAST tokens as the upvotes you are removing");
        baseTalkContract.voteLock();
    ...
    function removeDownvote(uint256 postId) public {
        ...
        require(baseTalkContract.balanceOf(msg.sender) >= posts[postId].downvotes[msg.sender], "Must hold at least as many BAST tokens as the downvotes you are removing");
        baseTalkContract.voteLock();
