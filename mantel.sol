...
mantleMeet deploys MMET, an ERC20 token implementing a 24 hr time lock owners can choose to place onto their tokens.

Tag deploys TAG, an ERC721 contract allowing holders of MMET to mint TAG, and allowing TAG owners certain permissions.
...
/**
 * @title Mantle Meet Token
 * @dev This contract deploys the MMET ERC20 token with an added 24hr time lock feature for participants.
 */
contract mantleMeet is ERC20 {
     ...
     */
    constructor() ERC20("mantleMeet", "MMET") {
...
contract Tag is ERC721Enumerable {
    ...
    mantleMeet public mantleMeetContract;
    /**
     * @dev Represents the details of a tag.
     */
    struct TagDetails {
        uint256 ethFee;            // Fee in ETH to use this tag
        uint256 tokenRequirement;   // Amount of mantleMeetContract required to hold for using this tag
    ...
    /**
     * @dev Initializes the Tag contract with a MMET token contract address.
     * @param _mantleMeetContract The address of the MMET token contract.
     */
    constructor(address _mantleMeetContract) ERC721("TagToken", "TAG") {
        mantleMeetContract = mantleMeet(_mantleMeetContract);
    }
    ...
    function createTag(string memory tagContent) public {
        require(mantleMeetContract.balanceOf(msg.sender) > 10, "Must hold at least ten MMET tokens");
    ...
    function updateTagFee(string memory tagContent, uint256 newEthFee) public {
        require(mantleMeetContract.balanceOf(msg.sender) > 0, "Must hold at least one MMET token");
        uint256 tokenId = uint256(keccak256(bytes(tagContent)));
    ...
    /**
     * @dev Updates the MMET token requirement for using a tag.
     * @param tagContent The content of the tag.
     * @param newTokenRequirement The new MMET token requirement.
     */
    ...
    function updateTokenRequirement(string memory tagContent, uint256 newTokenRequirement) public {
        ...
        require(ownerOf(tokenId) != address(0), "Tag does not exist");
        require(ownerOf(tokenId) == msg.sender, "You are not the owner");
        require(mantleMeetContract.balanceOf(msg.sender) >= newTokenRequirement);
     ...
     * @param tagContent The content of the tag
     * @return The ETH fee and MMET token holding requirement for the tag
     */
 ...
 * It is designed to work in tandem with two other contracts: mantleMeet and Tag 
...
contract Forum {
     ...
     * @param _mantleMeetContract Address of the mantleMeet contract.
     * @param _mantleMeetContract Address of the Tag contract.
     */

    constructor(address _mantleMeetContract, address _tagContract) {
        mantleMeetContract = mantleMeet(_mantleMeetContract);
    ...
    function tagPost(uint256 postId, string calldata tagContent) public {
        ...
        require(mantleMeetContract.balanceOf(msg.sender) >= TagContract.getTokenRequirement(tagContent), "Must hold more MMET tokens.");
        ...
        if (TagContract.ownerOf(id) != msg.sender && fee > 0 && !TagContract.isExemptFromTagFee(tagContent, msg.sender)) { 
            TagContract.transferFrom(msg.sender, TagContract.ownerOf(id), fee);
        }
    ...
    function upvotePost(uint256 postId) public {
        require(mantleMeetContract.balanceOf(msg.sender) > 0, "Must hold at least one MMET token to vote");
        ...
        uint256 amount = mantleMeetContract.balanceOf(msg.sender);
        mantleMeetContract.voteLock();
    ...
    function removeUpvote(uint256 postId) public {
        ...
        require(mantleMeetContract.balanceOf(msg.sender) >= posts[postId].upvotes[msg.sender], "Must hold at least as many MMET tokens as the upvotes you are removing");
        mantleMeetContract.voteLock();
    ...
    function removeDownvote(uint256 postId) public {
        ...
        require(mantleMeetContract.balanceOf(msg.sender) >= posts[postId].downvotes[msg.sender], "Must hold at least as many MMET tokens as the downvotes you are removing");
        mantleMeetContract.voteLock();
