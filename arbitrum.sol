...
arbinet deploys ARBN, an ERC20 token implementing a 24 hr time lock owners can choose to place onto their tokens.

Tag deploys TAG, an ERC721 contract allowing holders of ARBN to mint TAG, and allowing TAG owners certain permissions.
...
/**
 * @title Arbinet Token
 * @dev This contract deploys the ARBN ERC20 token with an added 24hr time lock feature for participants.
 */
contract arbinet is ERC20 {
     ...
     */
    constructor() ERC20("arbinet", "ARBN") {
...
contract Tag is ERC721Enumerable {
    ...
    arbinet public arbinetContract;
    /**
     * @dev Represents the details of a tag.
     */
    struct TagDetails {
        uint256 ethFee;            // Fee in ETH to use this tag
        uint256 tokenRequirement;   // Amount of arbinetContract required to hold for using this tag
    ...
    /**
     * @dev Initializes the Tag contract with a ARBN token contract address.
     * @param _arbinetContract The address of the ARBN token contract.
     */
    constructor(address _arbinetContract) ERC721("TagToken", "TAG") {
        arbinetContract = arbinet(_arbinetContract);
    }
    ...
    function createTag(string memory tagContent) public {
        require(arbinetContract.balanceOf(msg.sender) > 10, "Must hold at least ten ARBN tokens");
    ...
    function updateTagFee(string memory tagContent, uint256 newEthFee) public {
        require(arbinetContract.balanceOf(msg.sender) > 0, "Must hold at least one ARBN token");
        uint256 tokenId = uint256(keccak256(bytes(tagContent)));
    ...
    /**
     * @dev Updates the ARBN token requirement for using a tag.
     * @param tagContent The content of the tag.
     * @param newTokenRequirement The new ARBN token requirement.
     */
    ...
    function updateTokenRequirement(string memory tagContent, uint256 newTokenRequirement) public {
        ...
        require(ownerOf(tokenId) != address(0), "Tag does not exist");
        require(ownerOf(tokenId) == msg.sender, "You are not the owner");
        require(arbinetContract.balanceOf(msg.sender) >= newTokenRequirement);
     ...
     * @param tagContent The content of the tag
     * @return The ETH fee and ARBN token holding requirement for the tag
     */
 ...
 * It is designed to work in tandem with two other contracts: arbinet and Tag 
...
contract Forum {
     ...
     * @param _arbinetContract Address of the arbinet contract.
     * @param _arbinetContract Address of the Tag contract.
     */

    constructor(address _arbinetContract, address _tagContract) {
        arbinetContract = arbinet(_arbinetContract);
    ...
    function tagPost(uint256 postId, string calldata tagContent) public {
        ...
        require(arbinetContract.balanceOf(msg.sender) >= TagContract.getTokenRequirement(tagContent), "Must hold more ARBN tokens.");
    ...
    function upvotePost(uint256 postId) public {
        require(arbinetContract.balanceOf(msg.sender) > 0, "Must hold at least one ARBN token to vote");
        ...
        uint256 amount = arbinetContract.balanceOf(msg.sender);
        arbinetContract.voteLock();
    ...
    function removeUpvote(uint256 postId) public {
        ...
        require(arbinetContract.balanceOf(msg.sender) >= posts[postId].upvotes[msg.sender], "Must hold at least as many ARBN tokens as the upvotes you are removing");
        arbinetContract.voteLock();
    ...
    function removeDownvote(uint256 postId) public {
        ...
        require(arbinetContract.balanceOf(msg.sender) >= posts[postId].downvotes[msg.sender], "Must hold at least as many ARBN tokens as the downvotes you are removing");
        arbinetContract.voteLock();
